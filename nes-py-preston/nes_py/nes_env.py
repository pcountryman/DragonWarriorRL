...
# create a type for the extended memory vector from C++
MEMORY_VECTOR = ctypes.c_byte * 0x8000
...
class NESEnv(gym.Env):
    ...
    def __init__(self, rom_path):
        ...
        self.memory = self._memory_buffer()
    ...
    def _memory_buffer(self):
        ...
        address = _LIB.Memory(self._env)
        # create a buffer from the contents of the address location
        buffer_ = ctypes.cast(address, ctypes.POINTER(MEMORY_VECTOR)).contents
        # create a NumPy array from the buffer
        return np.frombuffer(buffer_, dtype='uint8')
